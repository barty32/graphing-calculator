...
import { ExpressionParser, ExpressionType, ParserFatalError, Severity, TokenType } from './parser.js';
...
(function (LineType) {
    ...
    LineType[LineType["function"] = 4] = "function";
})(LineType || (LineType = {}));
...
class Line {
    ...
    DOM = {
        ...
        audioBtn: undefined,
        fnType: undefined,
        sliderGroup: undefined,
        sliderSetup: undefined,
        slider: undefined,
        clipping: undefined
    };
    constructor(type) {
    ...
    addVariableSlider(parent) {
        ...
        this.DOM.sliderSetup.querySelector('input[inp-min]')?.addEventListener('input', (e) => {
            this.DOM.slider.min = e.target.value;
        });
        this.DOM.sliderSetup.querySelector('input[inp-max]')?.addEventListener('input', (e) => {
            this.DOM.slider.max = e.target.value;
        });
        this.DOM.sliderSetup.querySelector('input[inp-step]')?.addEventListener('input', (e) => {
            this.DOM.slider.step = e.target.value;
        });
        this.DOM.slider.addEventListener('input', () => {
            (this.DOM.subContainer?.querySelector('math-field')).setValue(this.variableName + '=' + this.DOM.slider?.value, { suppressChangeNotifications: false });
        });
    ...
    addPlaybackOptions(parent) {
        ...
        `;
        const dCurView = this.DOM.playbackOptions.querySelector(`#playback-current-view-${this.id}`);
        const dLoop = this.DOM.playbackOptions.querySelector(`#playback-loop-${this.id}`);
        const dStart = this.DOM.playbackOptions.querySelector(`#start-input-${this.id}`);
        const dEnd = this.DOM.playbackOptions.querySelector(`#end-input-${this.id}`);
        const dDownload = this.DOM.playbackOptions.querySelector(`#download-${this.id}`);
        this.DOM.clipping = this.DOM.playbackOptions.querySelector(`#clipping-${this.id}`);
...
DOM.optionsDegrees?.addEventListener('click', () => {
    for (const line in lines) {
        if (lines[line])
            lines[line].parser.rules.useDegrees = true;
...
DOM.optionsRadians?.addEventListener('click', () => {
    for (const line in lines) {
        if (lines[line])
            lines[line].parser.rules.useDegrees = false;
        lines[line]?.updateWorkerRules();
    }
    graph.draw(true);
});
