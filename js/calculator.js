...
class Line {
        ...
        if (this.type == LineType.expression || this.type == LineType.audio) {
            ...
            fileInput.addEventListener('change', (e) => {
                ...
                fileInput.files[0].text().then((jsonData) => {
                    ...
                    if (obj.format != "GraphingCore_v1.0")
    ...
    edit() {
        ...
        if (this.audioData) { //currentLine.audio
            document.querySelector(`#wave-select`).value = this.audioData.waveType; //currentLine.audio.waveType;
            for (const val in this.audioData) {
                if (val == 'waveType' || val == 'bias')
                    continue;
    ...
    toggleAudio() {
        ...
        const topBtn = document.querySelector('#btn-play');
        //play audio
...
document.querySelector('#wave-select')?.addEventListener('change', (e) => {
    if (currentEditedLine.audioData) {
        currentEditedLine.audioData.waveType = e.target.value;
        document.querySelector(`#expr-input-${currentEditedLine.id}`).disabled = currentEditedLine.audioData.waveType == 'custom' ? false : true;
    }
