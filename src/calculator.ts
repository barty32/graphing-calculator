...
const DOM = {
    playbackCurrentView: document.querySelector('#playback-current-view') as HTMLInputElement,
    playbackLoop: document.querySelector('#playback-loop') as HTMLInputElement,
    startInput: document.querySelector('#start-input') as HTMLInputElement,
    endInput: document.querySelector('#end-input') as HTMLInputElement,
...
var variables: { [index: string]: number } = {};
var idCounter = -1;
...
DOM.waveSelect?.addEventListener('change', (e) => {
    if (currentEditedLine.audioData) {
        ...
        const input = document.querySelector(`#expr-input-${currentEditedLine.id}`) as HTMLInputElement;
        if (currentEditedLine.audioData.waveType == 'custom') {
            input.disabled = false;
            if (input.value == '') graph.attachFn(currentEditedLine.id, () => undefined, ExpressionType.FUNCTION);
        }
        else {
            input.disabled = true;
        }
...
DOM.playbackCurrentView?.addEventListener('change', (e) => {
    ...
    if (currentEditedLine.audioData) currentEditedLine.audioData.currentView = DOM.playbackCurrentView.checked;
    currentEditedLine.audioDataChanged();
});

DOM.playbackLoop?.addEventListener('change', (e) => {
    if (currentEditedLine.audioData) currentEditedLine.audioData.loop = DOM.playbackLoop.checked;
    currentEditedLine.audioDataChanged();
});

DOM.startInput?.addEventListener('change', (e) => {
    if (currentEditedLine.audioData) currentEditedLine.audioData.start = parseFloat(DOM.startInput.value);
    currentEditedLine.audioDataChanged();
});

DOM.endInput?.addEventListener('change', (e) => {
    if (currentEditedLine.audioData) currentEditedLine.audioData.end = parseFloat(DOM.endInput.value);
    currentEditedLine.audioDataChanged();
});
